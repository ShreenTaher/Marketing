    ...
    <div class="m-portlet m-portlet--mobile">
          ...
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="createCompanyLabel">Create new City</h5>
              ...
              <div class="modal-body">
                <div class="form-group">
                  ...
                  <div class="form-group">
                    ...
                    <select class="form-control m-input" name="country_id" required data-parsley-required-message="kindly select country">
                        <option value="">Select</option>
                        @forelse($countries as $country)
                        <option value="{{ $country->id }}">{{ $country->en_name->name }}</option>
                        @empty
                        @endforelse
        ...
        <div class="m-portlet__body">
            ...
            <table class="table table-striped- table-bordered table-hover responsive" id="m_table_1">
                ...
                <tbody>
                @forelse($cities as $model)
                <tr>
                <td>{{$model->id}}</td>
                <td>{{ $model->en_name ? $model->en_name->name : '-' }}</td>
                <td>{{$model->ar_name ? $model->ar_name->name : '=' }}</td>
                <td>{{ $model->country != null ? $model->country->en_name->name : '-'}}</td>

                <td>{{ $model->is_active == 1 ? 'activated' : 'deactivated'}}</td>
                          ...
                          <div class="modal-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control m-input" name="country_id" required data-parsley-required-message="kindly select country">
                                        ...
                                        @forelse($countries as $country)
                                            <option value="{{ $country->id }}" @if($model->country){{$model->country->id == $country->id ? 'selected' : ''}}@endif>{{ $country->en_name->name }}</option>
                                        @empty
                                        @endforelse
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        type: 'POST',
                        headers: { 'Authorization': '{{$access_token}}'  },
                        url: '{{  app('shared')->get('base_url')}}' +'/setting/cities/activate/',
